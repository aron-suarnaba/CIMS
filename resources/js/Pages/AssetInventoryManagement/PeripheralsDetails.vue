...
const myBreadcrumb = [
    ...
    { label: 'Peripherals', url: route('peripherals.index') },
    { label: 'Details' },
];
...
const deleteItem = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('peripherals.destroy', id), {
                onBefore: () => {
                    Swal.fire({
                        title: 'Processing...',
...
const submit = () => {
    form.post(route('peripherals.issue', props.phone.id), {
        onSuccess: () => {
            const closeButton = document.querySelector(
                '#IssuePeripheralModal [data-bs-dismiss="modal"]',
            );
            if (closeButton) {
                closeButton.click();
            }
...
const returnSubmit = () => {
    returnform.post(route('peripherals.return', props.phone.id), {
        onSuccess: () => {
            returnform.reset();
            selectedReturnAcc.value = [];

            const closeButton = document.querySelector(
                '#ReturnPeripheralModal [data-bs-dismiss="modal"]',
            );
            if (closeButton) {
                closeButton.click();
            }
...
const updateSubmit = () => {
    updateForm.put(route('peripherals.update', props.phone.id), {
        forceFormData: true,
        onSuccess: () => {
            ...
            const closeButton = document.querySelector(
                '#UpdatePeripheralModal [data-bs-dismiss="modal"]',
            );
            if (closeButton) {
                closeButton.click();
            }
...
const openUpdateModal = (phone) => {
    ...
    const modalElement = document.getElementById('UpdatePeripheralModal');
    if (modalElement) {
        const modalInstance =
            window.bootstrap.Modal.getOrCreateInstance(modalElement);
        modalInstance.show();
    } else {
        console.error('Modal element #UpdatePeripheralModal not found');
    }
};
...
const openIssueModal = () => {
    const modalElement = document.getElementById('IssuePeripheralModal');
...
const openReturnModal = () => {
    const modalElement = document.getElementById('ReturnPeripheralModal');
    if (!modalElement) {
        console.error('ReturnPeripheralModal not found');
        return;
    }
...
const generateLogsheet = (id) => {
    window.open(
        `
         /AssetAndInventoryManagement/Peripherals/${id}/logsheet `,
        '_blank',
    );
};
...
onMounted(() => {
    ...
    document
        .getElementById('UpdatePeripheralModal')
...
<template>
                        ...
                        <div class="card-body">
                            ...
                            >
                                <div class="col-sm-12 col-md-6 mb-3">
                                    <BackButton
                                        @click.prevent="
                                            router.get(route('peripherals.index'))
                                        "
                                    />
                                </div>
                                <div
                                ...
                                <div class="flip-card-inner">
                                    ...
                                    <div class="flip-card-back d-flex flex-column align-items-center justify-content-center bg-white rounded shadow-sm border">
                                        <div class="p-3 text-center">
                                            <img :src="qr_code" alt="Asset QR Code" class="img-fluid mb-2" style="max-width: 140px;" />
    ...
    <!-- Issuance Modal -->
    <Modals
        id="IssuePeripheralModal"
        title="Issue Peripheral Asset"
    ...
    <!-- Return Modal -->
    <Modals
        id="ReturnPeripheralModal"
        title="Return Peripheral Asset"
        header-class="bg-primary text-white bg-gradient"
        layout="bento"
    >
        <template #body>
    ...
    <!-- Update Modal -->
    <Modals
        id="UpdatePeripheralModal"
        title="Update Peripheral Asset"
        header-class="bg-primary text-white bg-gradient"
        layout="bento"
    >
        <template #body>
