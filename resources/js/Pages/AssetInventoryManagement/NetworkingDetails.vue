...
const myBreadcrumb = [
    ...
    { label: 'Networking', url: route('networking.index') },
    { label: 'Details' },
];
...
const deleteItem = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('networking.destroy', id), {
                onBefore: () => {
                    Swal.fire({
                        title: 'Processing...',
...
const submit = () => {
    form.post(route('networking.issue', props.phone.id), {
        onSuccess: () => {
            const closeButton = document.querySelector(
                '#IssueNetworkingModal [data-bs-dismiss="modal"]',
            );
            if (closeButton) {
                closeButton.click();
            }
...
const returnSubmit = () => {
    returnform.post(route('networking.return', props.phone.id), {
        onSuccess: () => {
            returnform.reset();
            selectedReturnAcc.value = [];

            const closeButton = document.querySelector(
                '#ReturnNetworkingModal [data-bs-dismiss="modal"]',
            );
            if (closeButton) {
                closeButton.click();
            }
...
const updateSubmit = () => {
    updateForm.put(route('networking.update', props.phone.id), {
        forceFormData: true,
        onSuccess: () => {
            ...
            const closeButton = document.querySelector(
                '#UpdateNetworkingModal [data-bs-dismiss="modal"]',
            );
            if (closeButton) {
                closeButton.click();
            }
...
const openUpdateModal = (phone) => {
    ...
    const modalElement = document.getElementById('UpdateNetworkingModal');
    if (modalElement) {
        const modalInstance =
            window.bootstrap.Modal.getOrCreateInstance(modalElement);
        modalInstance.show();
    } else {
        console.error('Modal element #UpdateNetworkingModal not found');
    }
};
...
const openIssueModal = () => {
    const modalElement = document.getElementById('IssueNetworkingModal');
...
const openReturnModal = () => {
    const modalElement = document.getElementById('ReturnNetworkingModal');
    if (!modalElement) {
        console.error('ReturnNetworkingModal not found');
        return;
    }
...
const generateLogsheet = (id) => {
    window.open(
        `
         /AssetAndInventoryManagement/Networking/${id}/logsheet `,
        '_blank',
    );
};
...
onMounted(() => {
    ...
    document
        .getElementById('UpdateNetworkingModal')
...
<template>
                        ...
                        <div class="card-body">
                            ...
                            >
                                <div class="col-sm-12 col-md-6 mb-3">
                                    <BackButton
                                        @click.prevent="
                                            router.get(route('networking.index'))
                                        "
                                    />
                                </div>
                                <div
    ...
    <!-- Issuance Modal -->
    <Modals
        id="IssueNetworkingModal"
        title="Issue Networking Asset"
    ...
    <!-- Return Modal -->
    <Modals
        id="ReturnNetworkingModal"
        title="Return Networking Asset"
        header-class="bg-primary text-white bg-gradient"
        layout="bento"
    >
        <template #body>
    ...
    <!-- Update Modal -->
    <Modals
        id="UpdateNetworkingModal"
        title="Update Networking Asset"
        header-class="bg-primary text-white bg-gradient"
        layout="bento"
    >
        <template #body>
